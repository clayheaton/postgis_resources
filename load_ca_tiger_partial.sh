# PostGIS generates some of this automatically using the query here
# SELECT loader_generate_script(ARRAY['CA'], 'sh') AS result;
# load_ca_census_data.sh has TRACT, TABBLOCK, BG
...
# Possible error here with complaint that the shapefiles couldn't be imported so only the attributes were processed
